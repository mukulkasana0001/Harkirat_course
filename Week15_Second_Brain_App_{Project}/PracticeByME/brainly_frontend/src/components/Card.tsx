...
export const Card = (props: cardprops) => {
    ...
    return <>
        <div className="p-4 mb-1.5 bg-white rounded-md border-gray-200 max-w-72 m-3 border min-h-48 min-w-72">
            <div className="flex justify-between">

                {props.type == "twitter" &&<TwitterIcon/>}
                {props.type == "youtube" &&<YoutubeIcon/>}
                <div>{props.title}</div>
                <div onClick={props.onClick}><DeleteIcon/></div>
                {/* <div><ClipBoad/></div>
                <div><ShareIcon size ="sm"></ShareIcon></div> */}
            </div>
            ...
            <div className={`h-72 w-72  mt-4 items-center   ${props.type == "twitter" ? " overflow-y-scroll" : ""}`}>
                {props.type == "twitter" &&
                    (
                        ...
                        {/* <a href="https://twitter.com/Vinod_r108/status/1968630246933983683?ref_src=twsrc%5Etfw"> */}
                        </a></blockquote> 
